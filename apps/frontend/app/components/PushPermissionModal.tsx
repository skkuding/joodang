...
const messages: Record<
  string,
  { title: string; description: string; showFaqButton?: boolean }
> = {
  "push:unsupported": {
    title: "푸시 알림 미지원",
    description:
...
export function PushPermissionModal() {
  ...
  const iconSrc =
    eventKey === "push:subscribed"
      ? "/icons/icon_orange_noti.svg"
      : CautionIcon;
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="py-6">
        <div className="flex flex-col items-center justify-center">
          <Image
            src={iconSrc}
            alt={eventKey === "push:subscribed" ? "subscribed" : "caution"}
            width={48}
            height={48}
            className="mb-4"
          />
          <DialogTitle className="text-color-common-0 mb-4 text-center text-xl font-medium leading-7">
            {data?.title}
          </DialogTitle>
          <div className="text-color-neutral-40 mb-6 flex flex-col items-center text-center text-sm font-normal leading-tight">
